...
public class PausedScreenTitle implements Screen {
    SpriteBatch batch;
    Game game;
    Storage storage;
    ...
    public PausedScreenTitle(Game game, Storage storage) {
		this.game = game;
	...
	@Override
	public void show () {
        paused = new Sprite(new TextureRegion(new Texture(Gdx.files.internal("data/screens/optionscreentitle.png")), 0, 0, 800, 600));
    ...
    private void chooseInput() {
        if (goTitle.contains(Gdx.input.getX(), Gdx.input.getY())) {
            ...
            paused.setRegion(new Texture(Gdx.files.internal("data/screens/optionscreentitlehighlighted1.png")));
            Timer timer = new Timer();
            timer.scheduleTask(new Timer.Task() {
                @Override
                public void run() {
                    paused.setRegion(new Texture(Gdx.files.internal("data/screens/optionscreentitle.png")));
                }
            }, 0.1f);
            timer.scheduleTask(new Timer.Task() {
                @Override
        ...
        if (controls.contains(Gdx.input.getX(), Gdx.input.getY())) {
            ...
            buttonPressed = true;
            paused.setRegion(new Texture(Gdx.files.internal("data/screens/optionscreentitlehighlighted2.png")));
            Timer timer = new Timer();
            timer.scheduleTask(new Timer.Task() {
                @Override
                public void run() {
            ...
            final PausedScreenTitle screen = this;
            timer.scheduleTask(new Timer.Task() {
                @Override
                public void run() {
    ...
    private void setRectangles() {
        goTitle = new Rectangle(30 * widthRatio, 360 * heightRatio, 348 * widthRatio, 90 * heightRatio);
